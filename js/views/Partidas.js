...
const PartidasActions = {
  ...
  loadData: async () => {
    ...
    try {
      ...
      PartidasState.myMatches = todas.filter((m) => m.jugadores.some((p) => parseInt(p.usuario_id) === parseInt(userId)));
      PartidasState.openMatches = todas.filter((m) =>
        m.tipo === "public" && m.estado !== "cerrada" && m.jugadores.length < 4 && !m.jugadores.some((p) => parseInt(p.usuario_id) === parseInt(userId))
      );
    } catch (e) {
  ...
  joinMatch: async (match) => {
    if (confirm("¿Quieres unirte a esta partida?")) {
      ...
      if (res.success) { alert("¡Te has unido!"); PartidasActions.loadData(); }
  ...
  leaveMatch: async (match) => {
    if (confirm("¿Seguro que quieres salirte de esta partida?")) {
      try {
        ...
        if (res.success) { await PartidasActions.loadData(); alert("Has salido de la partida"); }
        else { alert(res.error); }
  ...
  view: () => {
    ...
    return m("div", { class: ThemeActions.getMainClasses() }, [
      ...
      m("div", { class: "page-content container", style: "padding-top: 20px;" }, [
        /* --- Control de Acceso --- */
        !Auth.user ? m("div", { class: "empty-state", style: "text-align: center; padding: 60px;" }, [
          ...
          m("button", { class: "btn-primary", onclick: () => m.route.set("/login") }, "INICIAR SESIÓN")
        ]) : [
          m("div", { class: "page-header", style: "margin-bottom: 30px;" }, [
            m("div", { style: "display:flex; justify-content:space-between; align-items:center" }, [
              m("div", [
                ...
                m("p", { class: "page-date", style: "color: var(--text-muted); text-transform: capitalize;" },
                  PartidasState.currentDate.toLocaleDateString("es-ES", { weekday: "long", day: "numeric", month: "long" })
                ),
              ]),
              m("div", { style: "display:flex; gap:10px" }, [
                m("button", { class: "date-nav-btn", onclick: () => PartidasActions.changeDate(-1) }, "←"),
                m("button", { class: "date-nav-btn", onclick: () => PartidasActions.changeDate(1) }, "→"),
              ]),
            ]),
          ]),
          ...
          PartidasState.loading
            ...
            : PartidasState.activeTab === "mine"
              ? (PartidasState.myMatches.length === 0
                ...
                : m("div", { class: "match-list", style: "display: flex; flex-direction: column; gap: 15px;" },
                  PartidasState.myMatches.map((match) => m(MatchCard, { match, isMyMatch: true }))
                ))
              : (PartidasState.openMatches.length === 0
                ...
                : m("div", { class: "match-list", style: "display: flex; flex-direction: column; gap: 15px;" },
                  PartidasState.openMatches.map((match) => m(MatchCard, { match, isMyMatch: false }))
                ))
        ]
      ])
    ]);
